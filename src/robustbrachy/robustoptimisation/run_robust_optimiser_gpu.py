...
def run_robust_optimiser_program_gpu(
    file_rp,
    file_rs,
    file_rd,
    file_source_data,
    ...
    plan_parameters = to_cpu(plan_parameters)
    ...
    if not isinstance(results.X, np.ndarray):
        ...
        (
            ...
            cp.array(dwell_times_pareto_front),
            cp.array(all_dose_per_dwell_per_vol_pt[0]),
            cp.array(all_dose_per_dwell_per_vol_pt[1]),
            cp.array(all_dose_per_dwell_per_vol_pt[2]),
        ...
        # send arrays to GPU.
        # float32 is used since it is more memory efficient, and GPU memory is generally limited.
        plan_parameters = to_gpu(plan_parameters)
        ...
        (
            ...
            plan_parameters,
            cp.array(dwell_times_pareto_front),
            cp.array(all_nominal_dvhs_pareto_front),
        )
...
__all__ = ["run_robust_optimiser_program_gpu"]
