...
def robust_measure_array_cpu(
    ...
    # arrays initiated
    all_D90_pop = np.zeros(
        (optimiser_parametres["no_of_runs"], len(dwell_times_pareto_front)),
        dtype=np.float32,
    )
    all_DmaxU_pop = np.zeros(
        (optimiser_parametres["no_of_runs"], len(dwell_times_pareto_front)),
        dtype=np.float32,
    )
    all_DmaxR_pop = np.zeros(
        (optimiser_parametres["no_of_runs"], len(dwell_times_pareto_front)),
        dtype=np.float32,
    )
    dose_per_vol = np.empty(
    ...
    for outer_itr in range(0, int(no_outer_itr)):
        ...
        all_volumes = np.zeros(
            (no_of_inner_itr, len(dwell_times_pareto_front), 2), dtype=np.float32
        )
        ...
        for inner_itr in range(0, no_of_inner_itr):
            ...
            changed_structures = copy_structures_cpu(
            ...
            dose_per_dwell_per_vol_pt_prostate_robust_eval = (
                TG43calc_dose_per_dwell_per_vol_pt_cpu(
            ...
            all_DVHs_pop = np.array(all_DVHs_pop, dtype=np.float32)
            # get volumes of OAR so we can get max doses and repeat into an array
            urethra_volume = volume_from_slices_vectorised_cpu(
                changed_structures["changed_urethra_contour_pts"]
            )
            rectum_volume = volume_from_slices_vectorised_cpu(
                changed_structures["changed_rectum_contour_pts"]
            )
            volumes = np.array([urethra_volume, rectum_volume])
            volumes = volumes * np.ones(
            ...
            vol_array = np.array(vol_array, dtype=np.float32)
        ...
        D90_pop, DmaxU_pop, DmaxR_pop = get_metrics_for_population_RE_array_cpu(
    ...
    mask_passed_DmaxR = ((DmaxR_pop - 13) <= 0).astype(np.bool_)
    mask_passed_DmaxU = (
    ...
    D90_passrate = (
        np.sum(mask_passed_D90, axis=0).astype(np.float32) / len(D90_pop) * 100
    )
    DmaxU_passrate = (
        np.sum(mask_passed_DmaxU, axis=0).astype(np.float32) / len(DmaxU_pop) * 100
    )
    DmaxR_passrate = (
        np.sum(mask_passed_DmaxR, axis=0).astype(np.float32) / len(DmaxR_pop) * 100
    )

    all_passrate = (
        np.sum(mask_passed_all, axis=0).astype(np.float32) / len(DmaxR_pop) * 100
    )
